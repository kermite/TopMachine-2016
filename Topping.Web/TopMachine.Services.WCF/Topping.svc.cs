...
using Topping.Web.Security;
using Topping.Web.Security.db4o;
using System.Security;
using Topping.Core.Logic.DB4o;
using Topping.Core.Logic.Interface;


namespace TopMachine.Services.WCF
{
    public class Topping : IProxy
    {
        ...
        {
            return GameLocalState.Instance;
        }
        ...
        public Player CheckAccessAdmin()
        {
           ...
           db4oProviders.User us_server = ta.GetUser(p.Pseudo); 
        ...
        #region Login
        private UserDto ConvertUserServerClient(db4oProviders.User us, db4oProviders.EnrolledUser en) 
        {
            UserDto us_result = new UserDto();

            us_result.UserName = us.UserName;
            us_result.FirstName = us.FirstName;
            us_result.LastName = us.LastName;  
            us_result.Email = us.Email;
            us_result.LastLoginDate = us.LastLoginDate;  
            if (en != null)
            {
                us_result.Roles = en.Roles;
            }
            return us_result;

        }
        private db4oProviders.User ConvertUserClientServer(UserDto us)
        {          
          db4oProviders.User us_server;
          us_server = new ToppingAccessor().GetUser(us.UserName);
              if (us_server == null)
              {
                  us_server = new db4oProviders.User();
              }
        ...
        private db4oProviders.EnrolledUser getEnRolledUser(db4oProviders.User us) 
        {
            db4oProviders.EnrolledUser en;
            en = new ToppingAccessor().getEnrolledUser(us);
        ...
        public List<UserDto> GetUsers()
        {
            ...
            CheckAccessAdmin();
            List <db4oProviders.User> lst = new ToppingAccessor().GetUsers();

            foreach (var item in lst)
            {
                db4oProviders.EnrolledUser en = new ToppingAccessor().getEnrolledUser(item);
                lst_result.Add(ConvertUserServerClient(item,en)); 
            }
        ...
        {
            CheckAccessAdmin();
            db4oProviders.User us_server = ConvertUserClientServer(us);
            return new ToppingAccessor().ChangedPwdUsers(us_server,NewPwd);
        }
        ...
        public bool UpdateUser(UserDto us)
        {
            ...
            db4oProviders.User us_server = ConvertUserClientServer(us);
            db4oProviders.EnrolledUser en = null;
            if (us_server.PKID != Guid.Empty)
            {
                en = getEnRolledUser(us_server);

            }
         
            return new ToppingAccessor().UpdateUsers(us_server,us.Roles,en);
        }
        ...
        public bool DeleteUser(UserDto us)
        {
            ...
            return new ToppingAccessor().DeleteUsers(us_server);
        }
        ...
        public Guid Login(string userName, string passWord, int version)
        {
            ...
            try
            {   
                db4oMembershipProvider mbp = new db4oMembershipProvider();
             
               // SQLiteMembershipProvider mbp = new SQLiteMembershipProvider();

                if (mbp.ValidateUser(userName, passWord))
                {
