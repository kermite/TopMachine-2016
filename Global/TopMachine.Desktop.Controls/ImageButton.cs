...
namespace TopMachine.Desktop.Controls
{
    ...
    public class ImageButton : System.Windows.Forms.Button
    {
        ...
        private Color oColor = Color.Transparent;                             // Centre colour for Path Gradient brushed
        [Bindable(true), Category("Button Appearance"),
        DefaultValue(false),
        ...
        private String buttonText { get { return Text; } }
        ...
        {
            if (isMouseIn && oColor != Color.Transparent)
            {
                buttonColor = this.oColor;
            }
            else
            {
        ...
        protected void DrawImage(Graphics g, Rectangle textRect)
        {
            if (Image == null) return;
            switch (ImageAlign)
            {
                case ContentAlignment.MiddleCenter:
                    int x = (textRect.Width - this.Image.Width) / 2;
                    int y = (textRect.Height - this.Image.Height) / 2;
                    g.DrawImage(Image, textRect.X + x, textRect.Y + y, Image.Width, Image.Height);
                    break;
                case ContentAlignment.MiddleLeft:
                    int xx = bevelHeight + bevelDepth + 3;
                    int yy = (textRect.Height - this.Image.Height) / 2;
                    g.DrawImage(Image, textRect.X + xx, textRect.Y + yy, Image.Width, Image.Height);
                    break;
            }
