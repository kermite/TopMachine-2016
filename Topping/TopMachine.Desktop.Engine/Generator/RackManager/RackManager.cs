...
namespace TopMachine.Topping.Engine.GameController.RackManager
{
    public class RackManager : IRackManager
    {
        ...
        #region Constructos
        public RackManager(GameCfg gc)
        {
            this.gc = gc;
            init();
        }

        public RackManager(GameCfg gc, CRack Track, ShuffleBag Tbag)
        {
            this.gc = gc;
            init();
            copy(Track, Tbag);
        ...
        public void ChooseTirage(bool joker, int intMinLettres)
        {
            ...
            for (int i = 0; (!IsBagEmpty()) && (i < intMinLettres); i++)
            {
                if (joker)
                {
                    ...
                    {
                        if (bag.ntiles() == bag.joker) //bag.tbag.tiles[0])
                        {
                            break;
                        }
        ...
        { // teste si on est arrive a la fin du jeu => plus de voyelle ou plus de consonne
            ...
            Debug.WriteLine("");
            Debug.WriteLine("v(" +vTotal + ") - c(" + cTotal + ") - j(" + jTotal +")");
