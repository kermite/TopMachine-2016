...
namespace TopMachine.Topping.Wcf
{
	...
	public class Topping : IProxy
	{
		...
		public Player CheckAccessAdmin()
		{
			...
			MembershipUser us_server = ta.GetUser(p.Pseudo);
		...
		#region Login
		private UserDto ConvertUserServerClient(MembershipUser us)
		{
			UserDto us_result = new UserDto();
			aspnet_Users us_server = new ToppingAccessor().GetUserSql(us.UserName);
			...
			us_result.Roles = Roles.GetRolesForUser(us.UserName).ToList();
		...
		private string[] getEnRolledUser(MembershipUser us)
		{
			return new ToppingAccessor().getEnrolledUser(us);
		...
		public List<UserDto> GetUsers()
		{
			...
			CheckAccessAdmin();
			List<MembershipUser> lst = new ToppingAccessor().GetUsers();
		...
		public bool ChangedPwdUser(UserDto us, string NewPwd)
		{
			...
			return new ToppingAccessor().ChangedPwdUsers(Membership.GetUser(us.Username), NewPwd);
		}
		...
		{
			CheckAccessAdmin();
			MembershipUser us_server = Membership.GetUser(us.Username);
